...
export function writingPracticeView(terms, memberId) {
  ...
  return '<header class="page-head"><span>LUYỆN VIẾT · 书写练习</span><h1>Luyện viết từ vựng</h1><p>Xem hoạt ảnh từng nét rồi thực hành viết ngay bên dưới. Mục Chiết Tự có không gian riêng trong thanh bên giao diện PC.</p></header>' +
    ...
    '<section class="writing-layout"><div class="panel writing-picker"><div class="writing-panel-head"><div><span class="writing-kicker">KHO TỪ VỰNG</span><h2>Chọn từ để viết</h2></div><span class="writing-count">' + terms.length + ' từ</span></div>' +
      ...
      '<div class="writing-section-head"><div><span class="writing-step">BƯỚC 1</span><strong>Quan sát nét viết</strong><small>Xem nét chữ chạy lặp lại để làm mẫu viết theo</small></div><button type="button" class="writing-animation-toggle" id="writingAnimationToggle" aria-pressed="false">Tạm dừng</button></div>' +
      ...
      '<section class="panel writing-practice-card"><div class="writing-section-head"><div><span class="writing-step">BƯỚC 2</span><strong>Thực hành viết</strong><small>Viết theo nét mẫu vừa quan sát</small></div><button type="button" class="writing-guide-toggle" id="writingGuideToggle">' + (showGuide ? 'Ẩn chữ mờ' : 'Hiện chữ mờ') + '</button></div>' +
